...
if __name__ == '__main__':
    ...
    input_test_images = '../data/raw/stage1_test'
    ...
    erosion_factor = 6 # 9 default value describing extent of erosion
    variable_erosion_factor = True # whether to use a different erosion factor depending on nuclei size
    medium_erosion_factor = 4 # 5 erosion factor when the nuclei size is smaller than the medium threshold
    ...
    for image_id in imdirs:
        ...
        if marker_erosion_type == 'normal':
            ...
            preprocessed_mask = numpy.copy( nuclei_mask )

            # subtract sobel edges
            if not sobel_threshold is None:
                sobel_image = skimage.filters.sobel( skimage.color.rgb2gray( image_array ) )
                preprocessed_mask[ numpy.array( sobel_image ) > sobel_threshold ] = 0

            # create a segmentation based on connected algorithm
            labels = skimage.measure.label( preprocessed_mask )
            ...
            # run erosion on the entire mask
                ...
                centers = skimage.morphology.erosion( preprocessed_mask, skimage.morphology.disk( erosion_factor ) )
