...
""" Batch script to extract and save negative images from split directories
...
from preprocess import extract_roi, generate_negative


class extract_split_roi( object ):

    def __init__( self, split_path, roi_path, padding=0, fixed_size=None, fixed_1x1_ratio=False, negative_size=None, number_of_neg_per_pos=1, check_neg_against_pos=False, neg_buffer_check_size=0 ):
    ...
    def create_roi_from_split( self, dirname ):
        ...
        for image_id in os.listdir( os.path.join( self.split_path, dirname ) ):
            ...
            positive.set_image_with_png( image_png )
            negative.set_image_with_png( image_png )

            # store some general information
            image_height, image_width = positive.image.shape[ 0:2 ]
    ...
    def negative_handling( self, negative_obj, dirname, image_id, mask_id, suffix='', roi_center_list=None ):
        ...
        :param negative_obj: The call to the ExtractROI class
        :param dirname: Name of the directory (e.g 'train', 'test')
        :param image_id: The hash-like id of the parent image
        :param mask_id: The hash-like id of the mask image
...
if __name__ == '__main__':
    ...
    roi_path = os.path.join( '..', 'data', 'neg_08' ) # 'roi' or 'small_roi'
